...
void bm_test_worker()
{
    ...
    const auto id1 = bm.add_button("5", "1");
    const auto id2 = bm.add_button("4", "3");
    const auto id3 = bm.add_button("3", "5");
    const auto id4 = bm.add_button("2", "4");
    const auto id5 = bm.add_button("1", "2");

    auto& view = bm.get_view();

    bm.sort<name_type>(std::less<>());
    auto v1 = std::vector{id5, id4, id3, id2, id1};
    assert(view == v1);

    bm.sort<filename_type>(std::less<>());
    auto v2 = std::vector{id1, id5, id2, id4, id3};
    assert(view == v2);

    bm.sort<identifier_type>(std::less<>());
    auto v3 = std::vector{id1, id2, id3, id4, id5};
    assert(view == v3);

    bm.sort<name_type>(std::greater<>());
    std::ranges::reverse(v1);
    assert(view == v1);

    bm.sort<filename_type>(std::greater<>());
    std::ranges::reverse(v2);
    assert(view == v2);

    bm.sort<identifier_type>(std::greater<>());
    std::ranges::reverse(v3);
    assert(view == v3);


    const auto id6 = bm.add_button("1");
...
void button_test_worker()
{
    ...
    PlayButton button("test1", 0);
    PauseButton pause("test2", 0);
    ResumeButton resume("test3", 0);
    StopButton stop("test4", 0);
    ...
    button.interact(controller.get());
